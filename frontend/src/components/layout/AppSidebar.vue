...
import { pluginNav } from '@/pluginkit/registry'
import { useEnabledPluginsStore } from '@/pluginkit/enabled'
import type { PluginNavItem } from '@/pluginkit/types'
...
// 风控中心已收编为 content-moderation 内建插件：导航项在原总开关之上叠加
// 插件 enabled 门控（fail-closed），插件停用时与路由守卫、后端 API 一致熄灭。
// 导航位置与原总开关语义保持不变（迁移期等价）。
const flagRiskControlPlugin = () =>
  flagRiskControl() && enabledPluginsStore.isEnabled('content-moderation')
...
// pluginNavItemsFor 将 enabled 插件贡献的导航项转换为核心 NavItem：
// labelKey 经 t() 解析、缺省图标补拼图图标。fail-closed：enabled 清单
// 未加载时 ids 为空集，不渲染任何插件项。
function pluginNavItemsFor(side: 'admin' | 'user'): NavItem[] {
  return pluginNav(side, enabledPluginsStore.ids).map(
    (item: PluginNavItem): NavItem => ({
      path: item.path,
      label: t(item.labelKey),
      icon: item.icon ?? PuzzlePieceIcon,
...
const userNavItems = computed((): NavItem[] =>
  finalizeNav(buildSelfNavItems(true)).concat(pluginNavItemsFor('user'))
)
...
const personalNavItems = computed((): NavItem[] =>
  finalizeNav(buildSelfNavItems(false)).concat(pluginNavItemsFor('user'))
)
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/risk-control', label: t('nav.riskControl'), icon: ShieldIcon, hideInSimpleMode: true, featureFlag: flagRiskControlPlugin },
  ...
  if (authStore.isSimpleMode) {
    ...
    // 插件管理是核心项（不 gated）；插件贡献的导航项按 enabled 门控
    filtered.push({ path: '/admin/plugins', label: t('nav.plugins'), icon: PuzzlePieceIcon })
    filtered.push(...pluginNavItemsFor('admin'))
  ...
  // 插件管理是核心项（不 gated）；插件贡献的导航项按 enabled 门控
  visible.push({ path: '/admin/plugins', label: t('nav.plugins'), icon: PuzzlePieceIcon })
  visible.push(...pluginNavItemsFor('admin'))
  for (const cm of customMenuItemsForAdmin.value) {
